...
```js
import { UButton, UCell, UField, UInput, UNavBar, UStack, UValidationMessage } from '@hia-uview/ui';
```

## Explicit plugin / 显式 plugin

Use `app.use(UView)` only when deliberate registration of every currently exported HIA component is appropriate. The current private collection is `u-button`, `u-cell`, `u-field`, `u-input`, `u-nav-bar`, `u-stack`, and `u-validation-message`. Importing the module alone has no global registration or style side effect.

仅当适合有意注册当前所有已导出 HIA 组件时才使用 `app.use(UView)`。当前私有集合为 `u-button`、`u-cell`、`u-field`、`u-input`、`u-nav-bar`、`u-stack` 和 `u-validation-message`。单独 import module 不会产生全局注册或样式副作用。
