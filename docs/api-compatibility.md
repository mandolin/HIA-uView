...
Text output provides a stable summary, including full API-item and P0 dispositions, evidence levels, and independent service counts, plus one line per component. Configure `report.format` as `json` when complete records within the declared inventory scopes are needed. A successful command means those declared scopes are structurally complete and internally consistent; it does **not** mean that all P1/P2 capabilities are delivered. The complete 1,740-item structural inventory currently reports 47 `compatible`, 339 `mapped`, and 1,354 `unsupported`. Within it, the 127/127 P0 summary is 43 `compatible`, 84 `mapped`, and 0 `unsupported`; all 127 are runtime-tested, none remains source-reviewed, and no P0 runtime-parity evidence remains outstanding. The two mapped service items are reported independently and enter neither component count nor API-item totals.

text 输出提供稳定汇总（包括完整 API item 与 P0 disposition、证据层级和独立 service 计数）及每组件一行的摘要。需要查看声明范围内的完整记录时，将 `report.format` 配置为 `json`。命令成功只表示这些声明范围结构完整、内部一致；并不表示全部 P1/P2 能力已经交付。完整 1,740 项结构盘点当前报告 47 个 `compatible`、339 个 `mapped` 与 1,354 个 `unsupported`。其中 127/127 项 P0 汇总为 43 个 `compatible`、84 个 `mapped` 与 0 个 `unsupported`；127 项均为 runtime-tested，没有项目仍处于 source-reviewed，也没有尚待补齐的 P0 runtime-parity 证据。两项 mapped service 会独立报告，既不进入组件 item 数，也不进入 API-item 总数。
...
Public composable services are not imperative component-ref methods and do not enter the 1,740 API-item baseline. The fixed upstream exposes `useModal` and `useToast` from its package root. HIA-uView now delivers same-named, explicitly scoped controllers through its root and pure `./services` entry, with opted-in component hosts, discriminated synchronous results, deterministic replacement/disposal/stale-request behavior, and no global/page discovery. Both records remain conservatively `mapped`, because callers must replace the upstream global/page event-bus, callback, and navigation semantics with HIA's caller-owned scope/host model; matching names do not make them compatible.

公开 composable service 不是组件 ref 的 imperative method，也不进入 1,740 项 API item 基线。固定上游从 package root 导出 `useModal` 与 `useToast`。HIA-uView 现已通过根入口与纯 `./services` 入口交付同名显式作用域 controller，配合 opt-in 组件 host、可判别同步结果，以及确定的替换/dispose/陈旧 request 行为，且不作全局/页面发现。两项仍保守标记为 `mapped`，因为调用方必须把上游 global/page event bus、callback 与导航语义改为 HIA 的 caller-owned scope/host 模型；同名不构成 compatible。
...
The current matrix marks precise HIA package declarations as `delivered` for the 12 comparison components in the bounded display/media/list batch; the HIA-owned `UCell` facade is outside the 99-name comparison. Their type-migration result remains `unsupported` with `HIA_COMPONENT_TYPE_PARITY_NOT_ASSESSED`, because a precise HIA contract is not a field-for-field uView-Pro type-equivalence claim. The offline Easycom package trial proves the private tarball can be configured and compiled; it does not turn the pre-release fragment into a package-stable public auto-registration contract, so the separate package Easycom status remains `not-delivered`.

当前矩阵把展示/媒体/列表受限批次中 12 个比较组件的精确 HIA 包声明标为 `delivered`；HIA 自有 `UCell` facade 不属于 99 名称比较集。这些组件的类型迁移结论仍以 `HIA_COMPONENT_TYPE_PARITY_NOT_ASSESSED` 保持 `unsupported`，因为精确的 HIA contract 并不等于与 uView-Pro 类型逐字段等价。离线 Easycom package trial 证明私有 tarball 可以配置并编译；它不会把预发布 fragment 变成包级稳定的公开自动注册契约，因此独立的 package Easycom 状态仍保持 `not-delivered`。
