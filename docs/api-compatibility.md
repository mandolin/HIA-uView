...
Text output provides a stable summary, including P0 dispositions, evidence levels, and independent service counts, plus one line per component. Configure `report.format` as `json` when complete records within the declared inventory scopes are needed. A successful command means those declared scopes are structurally complete and internally consistent; it does **not** mean that every source-reviewed item has runtime parity or that unsupported counts are zero. The current 127/127 P0 summary is 43 `compatible`, 82 `mapped`, and 2 `unsupported`: 71 are runtime-tested, 56 are source-reviewed, and 54 still explicitly require runtime-parity evidence. The two unsupported service items are reported independently and do not enter those 127 component API items.

text 输出提供稳定汇总（包括 P0 disposition、证据层级和独立 service 计数）及每组件一行的摘要。需要查看声明范围内的完整记录时，将 `report.format` 配置为 `json`。命令成功只表示这些声明范围结构完整、内部一致；并不表示每个 source-reviewed 项都已具备 runtime parity，也不表示 unsupported 数量为零。当前 127/127 项 P0 汇总为 43 个 `compatible`、82 个 `mapped` 与 2 个 `unsupported`：71 项为 runtime-tested，56 项为 source-reviewed，54 项仍明确需要 runtime-parity 证据。两项 unsupported service 会独立报告，不进入这 127 个组件 API item。
