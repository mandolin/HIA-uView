...
HIA-uView 当前接受官方 UniApp/Vue 编译依赖中已知的安全告警，以便为本仓、受信任的本地开发建立真实的 `mp-weixin` 编译证据。此决定是对风险的透明接受，不是“无风险”或“已修复”的声明。

HIA-uView currently accepts known security advisories in its official UniApp/Vue compilation dependencies so that trusted local development of this repository can obtain real `mp-weixin` compilation evidence. This is a transparent acceptance of risk; it is not a claim that the dependencies are risk-free or remediated.

## 已知事实 / Known facts

- 2026-07-29 的官方 registry 审计结果为 39 项：14 low、12 moderate、13 high、0 critical。告警包含 Vite、UniApp CLI 传递链及其传递依赖；可用自动修复会要求与官方锁定构建链不兼容的升级。
- The official registry audit on 2026-07-29 reported 39 findings: 14 low, 12 moderate, 13 high, and 0 critical. Findings include Vite, the transitive UniApp CLI chain, and its dependencies; available automated fixes require upgrades incompatible with the locked official toolchain.

- 传递依赖清单未发现 GPL、AGPL、LGPL、SSPL 或 BSL，但四个传递包缺少 package manifest 的许可证元数据；这不是“许可证已完全澄清”的声明。
- The transitive inventory found no GPL, AGPL, LGPL, SSPL, or BSL metadata, but four transitive packages lack license metadata in their package manifests; this is not a claim that licensing has been fully clarified.
...
该例外仅适用于此仓受信任内容的本地开发期编译和验证；依赖只存在于根 `devDependencies`，不是 UI 或 Tool 的运行时依赖，也不得作为发布包的 bundled dependency。

This exception applies only to local development-time compilation and validation of trusted content in this repository. The dependencies exist only in root `devDependencies`, are not UI or Tool runtime dependencies, and must not become bundled dependencies of a published package.

在该范围内，可以运行锁定的本地 compiler 完成 `mp-weixin` fixture build、静态检查、单元测试、文档检查和包内容检查。不得把 compile-only 结果表述为真机、读屏、焦点、跨端或生产安全证据。

Within this scope, the locked local compiler may run an `mp-weixin` fixture build, static checks, unit tests, documentation checks, and package-content checks. Compile-only results must not be represented as evidence for devices, screen readers, focus behavior, cross-platform support, or production security.
...
| --- | --- |
| C2 收尾、DCloud/Vite/Vue 升级或 lockfile 大幅变化 / C2 closeout, DCloud/Vite/Vue upgrade, or material lockfile change | 重新运行安全、许可证、编译和包内容审计；仅在兼容性证据充分时升级。 / Re-run security, license, compilation, and package-content audits; upgrade only with sufficient compatibility evidence. |
