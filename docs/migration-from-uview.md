...
2. 只比较应用实际拥有的组件。HIA-uView 当前有 89 个预发布组件契约，未实现 automatic-import、global service、picker、request、storage、router、form-engine 或业务模块兼容层；list、scroll、popup、toast、tabbar 和 pagination 仍是调用方受控的局部投影。`u-navbar` 与 `u-notice-bar` 是新建且独立的契约，不是 `u-nav-bar` 与 `u-notice` 的别名。
