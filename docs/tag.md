...
`UTag` presents caller-owned text with finite tokenized tone, size, shape, and appearance. Optional click and close controls report local intent only; the component never resolves a category registry, authorization, navigation, request, or business status and never hides itself.

`UTag` 以有限 token 化 tone、尺寸、形状与外观呈现调用方拥有的文字。可选 click 与 close control 只报告本地意图；组件绝不解析分类 registry、授权、导航、请求或业务状态，也绝不自行隐藏。
...
The tag projects only when both `visible` and `show` are true. The default slot replaces `text`. An enabled root click emits `click(originalEvent)`. An enabled close control emits payload-free `close()` and stops the same activation from becoming a root click. Neither event changes `visible` or `show`.

只有 `visible` 与 `show` 同时为真时才投影标签。默认 slot 替代 `text`。启用的根点击 emit `click(originalEvent)`。启用的关闭 control emit 无 payload 的 `close()`，并阻止同一次激活变成根 click。两个事件都不会改变 `visible` 或 `show`。
...
`disabled`, `show`, and `text` are reviewed compatible inputs. The default slot, click, and close now have runtime-tested bounded mappings, not upstream equivalence: upstream click/close report the configured `index`, while HIA reports the original click event and payload-free close respectively. `shape` and `size` require finite-value translation.

`disabled`、`show` 与 `text` 是已复核的 compatible 输入。默认 slot、click 与 close 现已有 runtime 测试的受限 mapping，但并非上游等价：上游 click/close 报告配置的 `index`，而 HIA 分别报告原始点击事件和无 payload close。`shape` 与 `size` 需要有限值转换。
...
Runtime tests cover dual visibility, slot/text precedence, numeric zero, Boolean/String disabled guards, original click identity, payload-free close, and click/close isolation. Package types constrain finite tokens. Compiler evidence does not prove authorization, category semantics, keyboard/screen-reader behavior, DevTools/device behavior, or cross-platform runtime.

Runtime 测试覆盖双可见性、slot/text 优先级、数字零、Boolean/String disabled guard、原始 click identity、无 payload close，以及 click/close 隔离。package 类型约束有限 token。compiler 证据不证明授权、分类语义、键盘/读屏行为、开发者工具/真机行为或跨端 runtime。
