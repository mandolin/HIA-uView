...
The fixture uses existing `UStack`, `UNavBar`, `UField`, `UInput`, `UCell`, `UValidationMessage`, `UEmpty`, `UModal`, `UNotice`, `UButton`, the P42 display components, the P43 controlled overlay/feedback/navigation components, and the P44 list/scroll/content components. It adds no auto-registration, global service, timer, router, or style injection.

fixture 使用既有的 `UStack`、`UNavBar`、`UField`、`UInput`、`UCell`、`UValidationMessage`、`UEmpty`、`UModal`、`UNotice`、`UButton`、P42 展示组件、P43 受控浮层/反馈/导航组件和 P44 列表/滚动/信息承载组件。它不新增自动注册、全局 service、timer、router 或样式注入。
...
This is not a HIA-uView-Biz module, an industry starter, a public data contract, or a backend adapter. The mock records are not customer, user, product, resource, or server-response data. The example has no network request, Directus integration, authentication, permission check, router, `uni.*` navigation, storage, cache, retry, virtualization, timer, queue, global service, image, icon, SVG, font, WXS, native plugin, or DOM observer; P43 loading/pagination/overlays and P44 list/scroll/disclosure state remain explicitly page-controlled.

这不是 HIA-uView-Biz 模块、行业 starter、公开数据契约或后端 adapter。mock 记录不是客户、用户、产品、资源或服务器响应数据。该示例没有网络请求、Directus 集成、身份、权限检查、router、`uni.*` 导航、storage、cache、retry、虚拟化、timer、queue、全局 service、图片、图标、SVG、字体、WXS、native plugin 或 DOM observer；P43 的 loading/分页/浮层和 P44 的列表/滚动/折叠状态仅由页面显式控制。
