...
/**
 * @lang zh-CN 验证组件只声明批准的 props、加载/禁用激活抑制和组件 token 消费；维护者源码不新增私有硬编码色值，已审计 MP-WEIXIN 条件编译的默认主题字面值规则除外。
 * @lang en Verifies that the component declares only approved props, suppresses loading/disabled activation, and consumes component tokens; maintainer-authored source adds no private hard-coded color except audited default-theme literal rules in MP-WEIXIN conditional compilation.
 */
test('keeps UButton inside its initial component contract', async () => {
  ...
  assert.match(styles, /\/\* #ifdef MP-WEIXIN \*\/[\s\S]*background: #0047ab;/);
});
