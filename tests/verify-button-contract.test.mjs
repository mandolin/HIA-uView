 ...
 * @lang zh-CN 验证首个 HiaButton 独立实现仍符合已批准 API、受限 locale 和 token 边界；真实 Vue/UniApp 处理另由 mp-weixin fixture 验证。
 * @lang en Verifies that the first independent HiaButton implementation remains within the approved API, constrained locale, and token boundary; real Vue/UniApp handling is verified separately by the mp-weixin fixture.
 */
 ...
 */
test('resolves only the documented HiaButton loading locales', () => {
 ...
 */
test('keeps HiaButton inside its initial component contract', async () => {
  const component = await readFile(resolve('HIA-uView-UI/src/components/hia-button/hia-button.vue'), 'utf8');
  const styles = await readFile(resolve('HIA-uView-UI/src/components/hia-button/hia-button.css'), 'utf8');
  ...
  assert.match(styles, /--hia-comp-button-primary-background/);
  assert.match(styles, /--hia-comp-button-min-height/);
