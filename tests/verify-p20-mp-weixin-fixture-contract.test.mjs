...
/**
 * @lang zh-CN 验证 fixture 的独立 package metadata 声明锁定微信编译器，并以 README 公开其临时校验、风险披露和非运行时边界。
 * @lang en Verifies that the fixture's independent package metadata declares the locked WeChat compiler and that its README discloses temporary validation, risk, and non-runtime boundaries.
...
test('keeps the mp-weixin fixture compiler declaration and internal validation boundary explicit', async () => {
  ...
  const [packageContent, readme, buildSource, manifestContent] = await Promise.all([
