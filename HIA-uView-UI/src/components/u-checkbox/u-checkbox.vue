...
<template>
  ...
  <button :class="checkboxClasses" :disabled="isDisabled" @click="handleChange">
    ...
    <!-- @lang zh-CN 默认 slot 仅替代调用方 label 的可见内容；它不接收 option 数据、scoped value 或业务上下文。 @lang en The default slot replaces only caller label content; it receives no option data, scoped value, or business context. <lang><zh-CN>slot 不取得选择状态或业务 authority。</zh-CN><en>The slot acquires neither choice state nor business authority.</en></lang> -->
...
const isChecked = computed(() => (
  groupContext ? groupContext.selectedValues.value.includes(props.value) : independentChecked.value
));
