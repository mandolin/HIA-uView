...
`check` validates the workspace and package-contract baseline. `npm test` uses the Node.js built-in test runner to verify component, Tool, theme, and documentation contracts. Runtime source and component APIs remain private and unpublished; the active compatibility profile is documented in [compatibility](docs/compatibility.md). The [component index](docs/components.md) links every current contract, including the P44 list, load-more, skeleton, collapse, swiper, horizontal-scroll, and sticky projections. These are independently implemented with Vue runtime, compiler evidence, and a limited local DevTools fixture observation; this does not imply a published API, device, accessibility-tree, asynchronous-validation, modal, timer, native-scroll, media, list-loading, or cross-platform guarantee.
