...
function formatInspectionDetails(details) {
  ...
  if (details.kind === 'api-compatibility') {
    ...
    for (const manifest of details.manifests) {
      ...
      // <lang><zh-CN>P0 语义行分别披露 source review、runtime test 与仍待 runtime parity 的数量，禁止用“已复核”偷换“已运行时等价”。</zh-CN><en>The P0 semantic line separately discloses source reviews, runtime tests, and remaining runtime parity so “reviewed” cannot be substituted for “runtime equivalent.”</en></lang>
      lines.push(`  - P0 semantics: ${manifest.summary.p0Semantics.reviewed}/${manifest.summary.p0Semantics.itemCount} reviewed; ${manifest.summary.p0Semantics.runtimeTested} runtime-tested; ${manifest.summary.p0Semantics.sourceReviewed} source-reviewed; ${manifest.summary.p0Semantics.runtimeParityRemaining} require runtime parity`);
      // <lang><zh-CN>service 行保持在 API item 总数之外，并分别披露无需迁移、显式映射与未交付数量。</zh-CN><en>The service line remains outside API-item totals and separately reports no-migration, explicitly mapped, and undelivered counts.</en></lang>
      lines.push(`  - services: ${manifest.summary.services.itemCount} items across ${manifest.summary.services.componentCount} components; ${manifest.summary.services.compatible} compatible; ${manifest.summary.services.mapped} mapped; ${manifest.summary.services.unsupported} unsupported`);
      for (const component of manifest.components) {
