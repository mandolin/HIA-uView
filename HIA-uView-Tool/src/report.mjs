...
function formatInspectionDetails(details) {
  ...
  if (details.kind === 'api-compatibility') {
    ...
    for (const manifest of details.manifests) {
      ...
      // <lang><zh-CN>service 行保持在 API item 总数之外，并明确当前未交付的上游 composable service 数。</zh-CN><en>The service line remains outside API-item totals and explicitly reports the currently undelivered upstream composable services.</en></lang>
      lines.push(`  - services: ${manifest.summary.services.itemCount} items across ${manifest.summary.services.componentCount} components; ${manifest.summary.services.unsupported} unsupported`);
      for (const component of manifest.components) {
